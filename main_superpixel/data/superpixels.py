...
def random_drop_edge(dataset, drop_percent):

    dataset_info = {}
    dataset_info['total_edge_num'] = 0
    dataset_info['total_node_num'] = 0
    dataset_info['new_node_num'] = 0
    dataset_info['new_edge_num'] = 0
    for idx ,graph in tqdm(enumerate(dataset)):
        
        graph_node_num = graph[0].number_of_nodes()
        graph_edge_num = graph[0].number_of_edges()
        dataset_info['total_node_num'] += graph_node_num
        dataset_info['total_edge_num'] += graph_edge_num
        if drop_percent > 0: 
            drop_edge_num = int(graph_edge_num * drop_percent)
            drop_list_idx = random.sample([i for i in range(graph_edge_num)], drop_edge_num)
            graph[0].remove_edges(drop_list_idx)

        graph_node_num = dataset[idx][0].number_of_nodes()
        graph_edge_num = dataset[idx][0].number_of_edges()
        dataset_info['new_node_num'] += graph_node_num
        dataset_info['new_edge_num'] += graph_edge_num

    dataset_info['total_graph_num'] = idx + 1

    pruning_percent = (dataset_info['total_edge_num'] - dataset_info['new_edge_num']) * 100 / dataset_info['total_edge_num']
    print("-" * 100)
    print("Ori Dataset | Graph:[{}] Nodes:[{}] Edge:[{}]"
        .format(dataset_info['total_graph_num'],
                dataset_info['total_node_num'],
                dataset_info['total_edge_num']))
    print("New Dataset | Graph:[{}] Nodes:[{}] Edge:[{}] Drop Edge:[{:.2f}%]"
        .format(dataset_info['total_graph_num'],
                dataset_info['new_node_num'],
                dataset_info['new_edge_num'],
                pruning_percent))
    print("-" * 100)
    return dataset, pruning_percent
...
class SuperPixDataset(torch.utils.data.Dataset):
    ...
    def __init__(self, name, args):
        ...
        if args.mode == "test":
            with open(data_dir + 'mnist_test.pkl', "rb") as f:
            ...
            with open(data_dir+name+'.pkl',"rb") as f:
                ...
                self.val = f[1]
                self.test = f[2]
                # file = open(data_dir + 'mnist_test.pkl', 'wb')
